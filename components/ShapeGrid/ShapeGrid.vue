...
/* ------------------------------------------------------------------ *
 * ShapeGrid — vendored from Vue Bits (TypeScript + Tailwind variant).
 *
 * Deviations from upstream, re-apply these if the component is
 * re-vendored via jsrepo:
 *   1. `vignetteColor` prop — upstream hardcodes the edge fade to
 *      '#14110E' (a warm brown), which fights any non-brown palette.
 *   2. `pointerTarget` prop — 'window' attaches the hover listeners to
 *      the window instead of the canvas and makes the canvas
 *      pointer-transparent, so the grid still reacts when it sits
 *      behind page content. Mirrors the pattern DotGrid.vue already
 *      uses in this project. Defaults to upstream 'canvas' behaviour.
 *   3. devicePixelRatio scaling — upstream draws at 1x, so the 1px
 *      cell strokes render soft on retina displays.
 *   4. prefers-reduced-motion — the grid stops scrolling (hover still
 *      works) instead of animating forever.
 * ------------------------------------------------------------------ */
...
function initCanvas() {
  ...
  // Logical (CSS pixel) size — the backing store is scaled by dpr.
  let cssWidth = 0;
  let cssHeight = 0;
  ...
  const resizeCanvas = () => {
    ...
    numSquaresX.value = Math.ceil(cssWidth / props.squareSize) + 1;
    numSquaresY.value = Math.ceil(cssHeight / props.squareSize) + 1;
  };
  ...
  const drawGrid = () => {
    ...
    if (isHex) {
      ...
      const rows = Math.ceil(cssHeight / props.squareSize) + 4;
      ...
      const cols = Math.ceil(cssWidth / props.squareSize) + 3;
      const rows = Math.ceil(cssHeight / props.squareSize) + 3;
  ...
  // In 'window' mode the canvas sits behind page content, so listening on
  // the canvas itself would only ever fire over uncovered gaps.
  const pointerHost: HTMLElement | Window = props.pointerTarget === 'window' ? window : canvas;
  const leaveHost: HTMLElement | Document = props.pointerTarget === 'window' ? document : canvas;

  window.addEventListener('resize', resizeCanvas);

  pointerHost.addEventListener('mousemove', handleMouseMove as EventListener);

  leaveHost.addEventListener('mouseleave', handleMouseLeave);
  ...
  cleanup = () => {
    ...
    pointerHost.removeEventListener('mousemove', handleMouseMove as EventListener);

    leaveHost.removeEventListener('mouseleave', handleMouseLeave);
...
<template>
  <canvas
    ref="canvasRef"
    class="block border-none w-full h-full"
    :class="pointerTarget === 'window' ? 'pointer-events-none' : ''"
  />
</template>
